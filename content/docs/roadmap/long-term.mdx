...
The protocol's success is not measured in revenue. It is measured in the number of integrators that depend on the on-chain Anchor without depending on the team. When an Entros gate is as routine as a token-balance check—when integrators reach for it without thinking about who maintains it—the protocol has done its job.
...
- **The bounded question.** The protocol answers "is this the same human, returning, over time" and nothing more. It does not turn into a KYC service, a credit-scoring service, or an identity-attestation service. The scope is the load-bearing claim.
- **Open programs, open SDK, open research.** Every layer is auditable in source. Every adversarial result is published. Every reference number in this documentation can be traced to a public artefact.
