...
<EntrosVerify
  ...
  onError={(err) => console.warn(err.reason)}  // wallet_rejected | validation_failed | network_error | timeout | popup_blocked | unknown
  onProgress={(p) => setStatus(p.status)}      // capturing → proving → submitting → attesting
