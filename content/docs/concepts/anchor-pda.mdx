...
The Anchor mint uses the Token-2022 `NonTransferable` extension. It is a one-supply, one-decimal token held in the user's associated token account. There is no transfer hook, no royalty extension, no metadata pointer to off-chain content. The mint exists to make the Anchor visible in any Token-2022-aware wallet UI; the authoritative score and metadata live in `IdentityState`.

## Three instructions

The `entros-anchor` program exposes three public instructions.
...
- **`update_anchor`**—Re-verification path. Validates a proof against the current commitment, advances the score, charges the fee, and rotates `previous_commitment` ← `current_commitment`.
- **`reset_identity_state`**—Recovery path. Zeroes the score and clears the fingerprint commitments after a 7-day cooldown, allowing a user who has lost their device drift envelope to start fresh. Charges the fee.
