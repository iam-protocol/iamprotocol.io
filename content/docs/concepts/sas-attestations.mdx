...
Walletless verifications produce attestations under the same schema, with `mode: "walletless"`. The attestation is the only on-chain record—there is no Anchor PDA, no compounding Trust Score, no persistent identity. Walletless mode is built for low-stakes filtering (CAPTCHA-equivalent surfaces) where a one-shot human signal is sufficient. For any gate that depends on the temporal-identity claim, integrators should require `mode === "wallet-connected"`.
